...
export function SignatureField({ field, value, onChange, onLayout, readOnly }: FieldComponentProps<string | null>) {
  ...
  return (
    <View onLayout={onLayout} style={{ marginBottom: 8 }}>
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        ...
        {value && (
          <Button mode="text" onPress={() => onChange(null)} disabled={readOnly}>
            Clear
          </Button>
        )}
      </View>
      ...
      {open && (
        <Modal transparent animationType="fade" onRequestClose={() => setOpen(false)}>
          <TouchableOpacity
            ...
            style={{ flex: 1, backgroundColor: '#0009', justifyContent: 'center', padding: 16 }}
          >
            <View style={{ backgroundColor: 'white', borderRadius: 12, padding: 12 }}>
              <SignatureCanvas
                ...
                clearText="Clear"
                confirmText="Save"
                webStyle={'.m-signature-pad--footer { display: flex; gap: 8px; }'}
                autoClear={false}
              />
            </View>
          </TouchableOpacity>
